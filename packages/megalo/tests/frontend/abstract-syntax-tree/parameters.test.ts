...
describe("parameterParserBuilder", () => {
  ...
  it("disambiguates hud_widget_set_meter signatures by reference type", () => {
    ...
    expect(off.parameters[1]).toMatchObject({
    ...
    expect(numbers.parameters[1]).toMatchObject({
  ...
  it("parses optional flag parameters when present", () => {
    ...
    expect(withFlag.parameters[1]).toMatchObject({
      kind: SyntaxKind.KEYWORD,
      value: "no_statistics",
    });
  ...
  it("parses optional parameters with values", () => {
    ...
    expect(parameters[5]).toMatchObject({
      kind: SyntaxKind.KEYWORD,
      value: "never_garbage",
    });
  });
  ...
  it("parses math operations as operator or keyword tokens", () => {
    ...
    expect(keyword.parameters[0]).toMatchObject({
      kind: SyntaxKind.KEYWORD,
      value: "set_to",
    });
    ...
    expect(operator.parameters[0]).toMatchObject({
      kind: SyntaxKind.KEYWORD,
      value: "+=",
    });
  });
