...
const lineStartLocation = (location: SourceCodeLocation): SourceCodeLocation => ({
    type: SourceLocationType.SOURCE_CODE,
    start: { offset: location.start.offset, line: location.start.line, column: 1 },
    end: { offset: location.start.offset, line: location.start.line, column: 1 },
});

const syntheticLineLocation = (line: number, near: SourceCodeLocation): SourceCodeLocation => ({
    type: SourceLocationType.SOURCE_CODE,
...
const isLeadingComment = (tokens: Tokens, index: number): boolean => {
    ...
    return previous === undefined || previous.location.start.line < token.location.start.line;
};
...
const findNextNonComment = (tokens: Tokens, fromIndex: number): Token | undefined => {
