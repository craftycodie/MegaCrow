...
type StringTableEntryNodeSymbol = {
  value: string;
  location: SourceCodeLocation;
};
type StringTableEntryNodeValue = ASTNode<SyntaxKind.QUOTED_STRING> & {
  value: string;
};
...
export type StringTableElementNode =
  ASTElementBase<ElementKind.STRING_TABLE> & {
...
export const stringTableParser = (
  ctx: ParserContext,
  elementToken: Token
  ...
  ctx.parseUntilEnd(() => {
    ...
    if (symbolToken.kind === TokenKind.Identifier) {
      ctx.symbolParser.addStringToScope({
        ...
        language: language.value as StringTableLanguage,
        content,
      });
    }
