import { Diagnostics, ObjectListLocation, SourceLocationType } from "../diagnostics";

// files that live under <megalo source folder>/object_lists/
export const enum ObjectListType {
    Objects = "objects",
    Weapons = "weapons",
    Vehicles = "vehicles",
    Equipment = "equipment",
...
export type ObjectLists = Readonly<Partial<Record<ObjectListType, readonly string[]>>>;

export const objectListLocation = (objectType: ObjectListType, index: number): ObjectListLocation => {
    return {
    ...
    public parse(text: string, diagnostics: Diagnostics): string[] {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (objectList.includes(line)) {
                diagnostics.addError(`Duplicate object "${line}" in object list`, objectListLocation(ObjectListType.Objects, i));
