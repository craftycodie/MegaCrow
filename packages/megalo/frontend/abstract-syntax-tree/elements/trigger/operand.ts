...
export const isComparisonOperatorName = (
  value: string
): value is ComparisonOperatorName =>
...
export const isComparisonToken = (kind: TokenKind, value: string): boolean =>
  ...
  (kind === TokenKind.Identifier && isComparisonOperatorName(value));
...
export const parseObjectOperand = (
  ...
  parseTypedOperand(ctx, anchor, ParameterType.Object);

export const parsePlayerOperand = (
  ctx: ParserContext,
  anchor: SourceCodeLocation
): ASTConditionOperandNode =>
  parseTypedOperand(ctx, anchor, ParameterType.Player);

export const parseTeamOperand = (
  ctx: ParserContext,
  anchor: SourceCodeLocation
): ASTConditionOperandNode =>
  parseTypedOperand(ctx, anchor, ParameterType.Team);

export const parseTimerOperand = (
  ctx: ParserContext,
  anchor: SourceCodeLocation
): ASTConditionOperandNode =>
  parseTypedOperand(ctx, anchor, ParameterType.Timer);
...
export const conditionOperandsParser =
  ...
  (ctx, anchor) =>
    parsers.map((parser) => parser(ctx, anchor));

/** Object list entry for object type names (objects.txt). */
export const ObjectTypeListParameter = ObjectListParameter(
