...
export class Lexer {
  ...
  private classifyNumeric = (
    ...
    let index = 0;
    if (
      text.charCodeAt(0) === CharCode.Minus ||
      text.charCodeAt(0) === CharCode.Plus
    ) {
      index = 1;
    }
    ...
    while (index < length) {
      ...
      if (code === CharCode.Semicolon) {
        ...
        while (index < length) {
          ...
          if (
            commentCode === CharCode.LineFeed ||
            commentCode === CharCode.CarriageReturn
          ) {
            break;
          }
          index++;
      ...
      if (code === CharCode.Quote) {
        ...
        while (index < length) {
          ...
          if (
            stringCode === CharCode.LineFeed ||
            stringCode === CharCode.CarriageReturn
          ) {
            break;
          }
          if (escaped) {
        ...
        const closed =
          index < length && source.charCodeAt(index) === CharCode.Quote;
      ...
      while (index < length) {
        ...
        if (
          (lexerCharsetTable[nextCode] & Char.Whitespace) !== 0 ||
          nextCode === CharCode.Quote
        ) {
          break;
        }
