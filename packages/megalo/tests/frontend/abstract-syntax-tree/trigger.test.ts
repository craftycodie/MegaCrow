...
const parseTriggerSource = (
  triggerName: string,
  body: string,
  setupSymbols = setupContext
) => {
...
describe("triggerParser", () => {
  ...
  it("reports unknown trigger statements", () => {
    ...
    expect(diagnostics.getErrors()[0]?.message).toContain(
      "Unrecognized element 'foo'"
    );
  });

  it("restores scope after parsing a trigger", () => {
    const { ctx, diagnostics, tokens } = setupContext(
  ...
  it("reports missing temporary initial values", () => {
    const { diagnostics } = parseTriggerElement(
      "\ttemporary player killing_player\n\taction end_round"
    );
