...
import { ASTErrorNode, SyntaxKind } from "..";
...
const parseHudWidgetEntry = (ctx: ParserContext): HudWidgetEntryNode => {
    ...
    if (positionToken.kind === TokenKind.Identifier) {
        ...
        ctx.diagnostics.addError(
            diagnosticMessages.expectedTokenKind(TokenKind.Identifier, positionToken.kind, positionToken.value),
...
export const hudWidgetsParser = (ctx: ParserContext, elementToken: Token): HudWidgetsElementNode => {
